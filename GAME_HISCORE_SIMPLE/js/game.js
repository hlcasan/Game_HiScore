...
var render_end = function () {
	...
	ctx.font = "100pt Acme";
	ctx.fillText("Out of time…", (canvas.width/2), (canvas.height/2)-120);

	ctx.fillStyle = "rgb(250, 250, 250)";
	//Player’s Score
	ctx.font = "20pt Acme";
	ctx.fillText("Your Score", (canvas.width/2)-200, (canvas.height/2)-60);
	ctx.font = "42pt Acme";
	ctx.fillText(pts, (canvas.width/2)-200, canvas.height/2);
	//High Score
	ctx.font = "18pt Acme";
	ctx.fillText("High Score", (canvas.width/2)+200, (canvas.height/2)-60);
	ctx.font = "36pt Acme";
	ctx.fillText(hScore, (canvas.width/2)+200, canvas.height/2);

	ctx.font = "18pt Acme";
	ctx.fillText("Click anywhere to play again", (canvas.width/2), (canvas.height/2)+60);
	...
	//I added a condition to store the score only if it is larger than the high score
	if (pts > hScore) {
